                    ...
                    {matches?.map((match) => (
                        <div className="match_left_container round">
                            <div className="teamOne_whole">
                                <span className="qualifiersteam_header"><span style={{fontFamily: "zoho_puviblack"}}>{match?.name}</span> | {formatDate(match?.schedule?.date)} | {match?.schedule?.from} - {match?.schedule?.to}</span>
                                <div className="qualifiers_teamOne">
                                    <div className={match.winner?.id !== undefined && match?.winner?.id === match.team1.id ? "qualifiersteamone_partOne qualifiers-match-winner" : "qualifiersteamone_partOne"}>
                                        <span>{match?.team1?.name}</span>
                                    ...
                                    <div className={match.winner?.id !== undefined && match?.winner?.id === match.team2.id ? "qualifiersteamone_parttwo qualifiers-match-winner" : "qualifiersteamone_parttwo"}>
                                        <span>{match?.team2?.name}</span>
